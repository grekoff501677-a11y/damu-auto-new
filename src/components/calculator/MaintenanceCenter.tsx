...
export function MaintenanceCenter() {
  ...
  const model = useMemo(() => DEMO_MODELS.find((m) => m.slug === modelSlug)!, [modelSlug])
  ...
  return (
    <div className="glass overflow-hidden rounded-3xl">
      ...
      <div className="border-b border-glass-border p-5 sm:p-6">
        ...
        {/* ── Model carousel (swipable) ── */}
        <div className="no-scrollbar mt-4 flex snap-x snap-mandatory gap-2 overflow-x-auto pb-1">
          {DEMO_MODELS.map((m) => {
            const active = m.slug === modelSlug
            return (
              <button
                key={m.slug}
                onClick={() => setModelSlug(m.slug)}
                className={cn(
                  'flex min-h-12 shrink-0 snap-start flex-col items-start justify-center rounded-xl border px-4 py-2 text-left transition-all duration-200 cursor-pointer',
                  active
                    ? 'border-accent bg-accent/10 shadow-[0_0_18px_-6px_rgba(196,154,69,0.7)]'
                    : 'border-input bg-surface/60 hover:border-accent/40'
                )}
              >
                <span className={cn('text-[10px] uppercase tracking-wide', active ? 'text-accent' : 'text-muted-foreground')}>
                  {m.brand}
                </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_1fr]">
        ...
        <div className="p-5 sm:p-6">
          {/* Horizontal mileage gauge (thumb-scrollable) */}
          <div className="no-scrollbar -mx-1 mb-6 overflow-x-auto px-1">
            <div className="relative flex min-w-max justify-between gap-6 sm:gap-2">
              ...
              <motion.div
                className="absolute left-0 top-[19px] h-px bg-accent shadow-[0_0_8px_rgba(196,154,69,0.6)]"
              ...
              {model.milestones.map((m, i) => (
                ...
                >
                  <span className={cn(
                    'flex h-10 w-10 items-center justify-center rounded-full border text-[11px] font-700 transition-all duration-300',
                    i === milestoneIdx
                      ? 'scale-110 border-accent bg-accent text-accent-foreground shadow-[0_0_16px_-2px_rgba(196,154,69,0.8)]'
                      : i < milestoneIdx
          ...
          {/* Parts: 2-col grid on mobile, list on desktop */}
              ...
              {milestone.parts.map((part, i) => (
                ...
                >
                  <div className="mb-2 flex items-center gap-2 lg:mb-0 lg:shrink-0">
                    ...
                    <span className={cn(
                      'rounded-full px-2 py-0.5 text-[10px] font-600 uppercase tracking-wide lg:hidden',
                      part.type === 'replace' ? 'bg-accent/10 text-accent' : 'bg-amber-400/10 text-amber-400'
                    )}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn(
                        'hidden shrink-0 rounded-full px-2 py-0.5 text-[10px] font-600 uppercase tracking-wide lg:inline',
                        part.type === 'replace' ? 'bg-accent/10 text-accent' : 'bg-amber-400/10 text-amber-400'
                      )}>
                        {part.type === 'replace' ? 'Замена' : 'Проверка'}
                      </span>
                    </div>
                    <p className="mt-0.5 line-clamp-1 text-xs text-muted-foreground">{part.spec}</p>
