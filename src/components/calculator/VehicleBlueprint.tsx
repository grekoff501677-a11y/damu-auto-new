    ...
    return (
      <div className={cn('relative', className)}>
        ...
        {lines.length > 0 && (
          <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            {lines.map((h) => {
              ...
              const stroke = on ? '#F8F4ED' : 'rgba(192,157,81,0.55)'
              return (
                <path key={h.id} d={leaderLinePath(h)!} fill="none" stroke={stroke} strokeWidth={1}
        ...
        {hotspots.map((h) => {
          ...
          // иерархия внутри палитры: белый (активно) → охра (декор) → голубой (выключено)
          const ring = on ? '#F8F4ED' : decorative ? 'rgba(192,157,81,0.8)' : 'rgba(169,185,198,0.45)'
          return (
            ...
            <div key={h.id} className="pointer-events-none absolute inset-0">
              ...
              >
                ...
                <span
                  ...
                  style={{ width: 44, height: 44, background: 'radial-gradient(circle, rgba(227,211,179,0.40), transparent 70%)' }}
                />
                {/* active pulse */}
                {on && (
                  <motion.span
                    className="pointer-events-none absolute rounded-full"
                    style={{ width: 26, height: 26, border: '1.5px solid rgba(227,211,179,0.6)' }}
              ...
              {h.label && h.label.length > 0 && (on || decorative || labelAtLine) && (
                <span
                  className={cn(
                    ...
                    on ? 'border-brand-beige/40 bg-brand-deep/75 text-brand-white'
                       : 'border-accent/30 bg-brand-deep/60 text-accent/90'
                  )}
      ...
      <svg viewBox="0 0 400 220" fill="none" className="h-full w-full">
        <defs>
          <linearGradient id="bp-stroke" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#E3D3B3" stopOpacity="0.95" />
            <stop offset="100%" stopColor="#C09D51" stopOpacity="0.7" />
          </linearGradient>
        ...
        <line x1="20" y1="196" x2="380" y2="196" stroke="rgba(248,244,237,0.08)" strokeWidth="1" strokeDasharray="2 6" />
        ...
        <motion.path
          ...
          stroke="rgba(248,244,237,0.18)" strokeWidth="1" strokeLinecap="round"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 0.8 }} />
        <motion.line x1="216" y1="110" x2="216" y2="160" stroke="rgba(248,244,237,0.10)" strokeWidth="1"
        ...
        {[120, 312].map((cx, i) => (
          <motion.g key={cx}
            ...
            <circle cx={cx} cy="170" r="11" stroke="rgba(248,244,237,0.25)" strokeWidth="1" />
          </motion.g>
        ))}
        ...
        {(Object.keys(NODES) as BodyNode[]).map((key) => {
          ...
          return (
            <g key={key}>
              {isOn && (
                <motion.circle cx={node.x} cy={node.y} r="18" fill="#C09D51" fillOpacity="0.14"
                  initial={{ scale: 0 }} animate={{ scale: [1, 1.35, 1] }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }} />
              )}
              <motion.circle cx={node.x} cy={node.y} r="4.5"
                animate={{ fill: isOn ? '#C09D51' : 'rgba(248,244,237,0.2)', filter: isOn ? 'url(#bp-glow)' : 'none' }}
