...
import type { Product, CarModel, BlogPost, Node3DRegion, Part3DPlacement } from '@/lib/types'
...
export async function getMaintenanceModels(): Promise<PublicMaintModel[]> {
  ...
  return ((models ?? []) as CarModel[]).map((m) => {
    ...
    return {
      ...
      parts3d: (Array.isArray(m.model_3d_parts) ? m.model_3d_parts : undefined) as Part3DPlacement[] | undefined,
    }
  })
}
...
// ============================================================
export async function getHomeSections(): Promise<ResolvedBlock[]> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('page_sections')
    .select('id, section_key, sort_order, is_visible, config')
    .eq('page', 'home')
  return resolveBlocks(data ?? []).filter((s) => s.visible)
}
