...
src/
  app/
    ...
    api/
      leads/route.ts        # POST заявки (валидация, rate-limit, consent, IP-hash)
      auth/logout/route.ts  # POST logout (Supabase signOut)
    admin/
...
- `src/components/ui/gradient-tracing.tsx` — добавлен по тех-заданию, но `HeroLogo` использует свою реализацию.
...
- Публичное чтение: `products` (is_available), `car_models` (is_active), `blog_posts` (published), `product_compatibility`, `maintenance_rules` — всем.
- `leads`: **anon может только INSERT**; `authenticated` — полный доступ.
- Остальное: `authenticated` (админ) — полный доступ (`auth.role() = 'authenticated'`).
...
5. Authentication → Users → **Add user** (email + пароль, ✅ Auto Confirm) — это логин в админку. Регистрации на сайте нет.
...
Координаты хотспотов — в **% от изображения**. Модель хотспота (`BlueprintHotspot` в `types`): `{ id, x, y, bodyNode?, label?, line?: { x2, y2, cx?, cy? } }`.
...
Логотип в правой части HERO (на мобиле — центр над заголовком). Эффект: **логотип в глубокой тени** (`brightness 0.32`), а **золотая «комета»** бежит по кольцу вокруг и **выхватывает лого из тени**:
- Комета: `conic-gradient` (яркая голова в 0°, один хвост, тающий по часовой), вращение **против часовой** (`rotate:-360`) → голова **ведёт**, хвост сзади. Два слоя (широкий blur-ореол + чёткая линия) на тонком кольце (`RING` маска `closest-side`).
- **Reveal**: поверх тёмной базы — натуральный логотип (`brightness 1.45`), видимый **только внутри движущегося мягкого спотлайта** (`SPOTLIGHT` radial-маска), который вращается синхронно с кометой; внутренний `<img>` контр-вращается (`rotate:+360`), чтобы оставаться ровным. Свет НЕ тонирует жёлтым — показывает настоящие цвета лого.
- Спотлайт компактный (виден контраст тень↔свет), привязан к голове кометы.
...
- **Header**: плавающий glass, marquee Geely Club сверху, логотип через `next/image` (`priority`), ссылки. На мобиле верхняя nav скрыта — её заменяет нижний таб-бар.
...
**Утилиты в globals.css:** `.glass`/`.glass-strong` (glassmorphism), `.glow-accent`, `.text-gradient`/`.text-gold-gradient`, `.grid-backdrop`, `.animate-marquee`, `.no-scrollbar`, `.touch-target` (48px), `.gpu`, глобальный `:focus-visible` ring (a11y), кастомный скроллбар, `prefers-reduced-motion`.
...
- **`/api/leads`** — серверная валидация (имя/телефон, regex телефона), очистка управляющих символов, in-memory rate-limit (5/мин на IP), **обязательная проверка `consent === true`** (иначе 422), хранение `consent_at` и **хэша IP** (не сырого) в `metadata`. Готов к webhook (закомментирован блок n8n).
...
**Осталось / на будущее:**
1. **Наполнить контент** через админку: правила ТО для Atlas/Coolray/Okavango; товары с фото (Cloudinary) и совместимостью; статьи блога; схемы авто (`/admin/blueprints`) для остальных моделей (картинка с Cloudinary + расставить хотспоты).
2. **Запустить миграцию 002** в актуальной БД, если ещё не запущена (иначе сохранение схем в админке падает).
...
**Открытых багов нет.** Сборка зелёная, TypeScript без ошибок.
