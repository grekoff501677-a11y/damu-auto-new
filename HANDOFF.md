...
1. Оптимизировать GLB (часто 50+ МБ → 2–3 МБ): `npx @gltf-transform/cli optimize in.glb out.glb --compress draco --texture-compress webp` (optimize включает simplify — геометрия чуть упрощается, для wireframe незаметно).
2. Залить на CDN: Cloudinary (raw/image upload, отдаёт с CORS `*`) либо Supabase Storage (`scripts/upload-3d.mjs`). Текстуры не нужны (рендерим wireframe) — но оптимизация всё равно ужимает геометрию.
3. Прописать URL: `UPDATE car_models SET model_3d_url = '...' WHERE slug = '...'`. Готово — Центр ТО сам покажет wireframe вместо 2D-схемы.
...
5c. SQL Editor → выполнить **`database/migrations/007_car_3d_model.sql`** — колонка `car_models.model_3d_url` + GLB-ссылка Coolray. Без неё Coolray покажет обычную 2D-схему (не сломано), а 3D-wireframe не появится.
