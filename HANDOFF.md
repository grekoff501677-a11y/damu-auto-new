...
- `src/components/calculator/Model3D.tsx`: Coolray оставлен в правильной ориентации через explicit rotation-fix `geely-coolray: [Math.PI / 2, 0, 0]`. Не убирать без визуальной проверки: общий auto-axis работает для Monjaro, но Coolray после него требует отдельного поворота.
- `src/components/calculator/Model3D.tsx`: обрезка wireframe по краям карточки была не near-plane, а CSS/canvas clipping: машина в профиль становилась шире видимой зоны. `Bounds` убран; после построения рёбер модель центрируется в wrapper-группе и масштабируется по длиннейшей стороне до `MODEL_MAX_SIZE = 3.4`; `OrbitControls minDistance = 6.2` не даёт приблизить модель обратно к краям.
