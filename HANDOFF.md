...
- Комета: **три** `conic-gradient`-дуги разной длины на кольцевых масках разной ширины (`RING_CORE` тонкая / `RING_SOFT` широкая, `closest-side`): широкий bloom (короткая дуга, blur 14px) + средний glow (blur 4px) + чёткое ядро (самая длинная дуга, до ~166°). У головы слои складываются (толсто, горячо), к концу остаётся одно тонкое ядро → **хвост сужается**. Поверх — «голова»: радиальное бело-золотое пятно на своём вращающемся слое (top 4.4% = центр кольцевой полосы). **Статичного кольца-«пути» НЕТ** (удалён по фидбеку — комета летит сама по себе). Вращение **против часовой** (`rotate:-360`) → голова **ведёт**, хвост сзади.
...
- Спотлайт с **длинной многостоповой полутенью** (стопы до 76%) — граница света рассеянная, без видимого края (тоже фидбек); привязан к голове кометы.
