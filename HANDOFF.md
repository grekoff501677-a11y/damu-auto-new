...
database/
  ...
  migrations/007_car_3d_model.sql   # car_models.model_3d_url + GLB-ссылки Coolray/Monjaro/Atlas/Okavango
  migrations/008_car_3d_nodes.sql   # car_models.model_3d_nodes jsonb (области узлов для роя частиц)
scripts/
...
- **Узлы ТО роем частиц** (`components/calculator/NodeSwarm.tsx` + `lib/node-regions.ts`): при выборе интервала активные `bodyNode` подсвечиваются **роем светящихся cyan-точек** (шейдерные additive-частицы, мерцание, плавный fade-in) в зоне узла на 3D-модели. Области (`Node3DRegion`: sphere/box в нормализованном пространстве) берутся из `car_models.model_3d_nodes` (миграция 008), а если пусто — **генерятся дефолтно из bbox** (`defaultRegions`, приблизительно по компоновке авто). Рои — дети Canvas в мировых координатах, совпадают с моделью при облёте камеры. ⚠️ TODO: **админ-редактор областей** (ставить/двигать/масштабировать сферы-боксы на модели, привязка к bodyNode, сохранение в `model_3d_nodes`) — пока не сделан, работают дефолты.
