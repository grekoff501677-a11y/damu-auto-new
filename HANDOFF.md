...
Логотип в правой части HERO (на мобиле — центр над заголовком). **Сценарий (по фидбеку)**: эмблема в естественной яркости + лёгкое золотое drop-shadow; **две тонкие кометы** рождаются на **6 часах**, поднимаются по обеим сторонам кольца (CW 6→3→12, CCW 6→9→12, `rotate: [0, ±180, ±180]`, доезжают к 88% цикла) и гаснут при встрече на 12 (`opacity times [0,0.06,0.88,1]`). Каждая комета = чёткая conic-линия (RING) + мягкий glow (RING_GLOW, blur 6). Цикл 4.4 с; скачок rotate при репите невидим (opacity=0). Выхватывания лого из тени и финальной заливки НЕТ (убрано по фидбеку — пользователь хотел только аккуратные линии). Фоновых radial-подложек тоже нет (давали «квадрат» свечения на краях блока).
