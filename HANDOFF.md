...
Логотип в правой части HERO (на мобиле — центр над заголовком). **Финальный вариант (выбран пользователем)**: тонкое GradientTracing-кольцо + лёгкая тень/подсветка:
- **GradientTracing** (`components/ui/gradient-tracing.tsx`, проп `responsive` — скейл через viewBox 600×600): золотой градиент-пульс по окружности r=282, strokeWidth 2.5, статичного трека нет (`baseColor="transparent"`). Контейнер кольца повёрнут на **+90°** — свет живёт на **6 часах** (по фидбеку: «с 9 на 6»).
- Эмблема слегка затенена (`brightness 0.9` + золотой drop-shadow), снизу — еле заметная тёплая подсветка (radial на 50% 92%, alpha 0.12). Пользователь просил «совсем немного» тени/света — не усиливать.
...
- Прежние версии анимации в git: комета со спотлайт-reveal — `aa758d7`, встречные дуги 12/6→3/9 с тенью — `1c1adbb`.
