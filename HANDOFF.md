  ...
  app/
    ...
    admin/
      ...
      (protected)/          # ← route group: ВЕСЬ guard здесь
        ...
        sections/           # КОНСТРУКТОР главной: порядок/видимость/тексты секций (SectionManager + actions)
...
- **page_sections** (миграция 005) — конструктор главной: `page('home'), section_key, sort_order, is_visible, config jsonb, updated_at`, UNIQUE(page,section_key). Набор `section_key` фиксирован в коде (`lib/page-sections.ts`), таблица лишь переопределяет порядок/видимость/тексты. Сайт работает и без таблицы (код падает на дефолты реестра) — таблица нужна, чтобы изменения из админки **сохранялись**.
...
5b. SQL Editor → выполнить **`database/migrations/005_page_sections.sql`** — таблица `page_sections` + сид 5 секций главной. Без неё `/admin/sections` (конструктор главной) откроется, но изменения порядка/видимости/текстов **не сохранятся** (сайт при этом работает на дефолтах). В свежем `schema.sql` уже учтено.
...
- `getHomeSections()` → секции главной: `resolveSections()` мерджит строки `page_sections` поверх реестра `page-sections.ts` (каждая секция всегда есть — graceful), фильтрует видимые, сортирует по `sort_order`. Главная (`page.tsx`) рендерит их по `section_key` через мапу рендереров; интерактивная начинка блоков (HeroLogo, MaintenanceCenter, MacroCompareSlider, LeadForm) — в коде, из config берутся только тексты.
...
Логотип в правой части HERO (на мобиле — центр над заголовком). **Финальный вариант (выбран пользователем — база коммита `a532d15` + усиленное выхватывание)**: эмблема в тени (`brightness 0.4`); по кольцу бежит **тонкая чёткая золотая линия-пульс** (`GradientTracing`, `components/ui/gradient-tracing.tsx`, проп `responsive`, strokeWidth 2.5, окружность r=282, цикл 4 с, контейнер повёрнут на −90° → пульс на 6 часах). Снизу — **мягкое выхватывание лого из тени**: тёплый radial-glow (GLOW) + яркая копия эмблемы (`brightness 1.4`) под radial-маской SPOT (длинная полутень до 76%, гаснет задолго до краёв — без «квадратов»). По фидбеку выхватывание сделано заметнее, чем в исходном `a532d15` (там было слабым).
