...
- `src/components/calculator/Model3D.tsx`: **ориентация полностью генерик** (per-model rotation-fix `geely-coolray` УБРАН — он переворачивал Coolray). Алгоритм: наименьшая ось bbox = высота авто → ставится вертикалью. Работает для всех (Monjaro Z-up FBX, Coolray Y-up — оба «на колёсах»), без хаков по slug.
