...
**Конструктор `/admin/sections`** ([SectionManager.tsx](src/app/admin/(protected)/sections/SectionManager.tsx)): drag-reorder (motion `Reorder` + `useDragControls`, тянется за ручку-grip; на `onDragEnd` → `reorderSections`), тумблер видимости, раскрывающийся редактор полей (типы поля в реестре: `text`/`textarea`/`toggle`/`list`), live-превью главной в `<iframe src="/">` с переключателем Десктоп/Моб (ширина 390px) — перезагружается по `nonce` после каждого действия. Server actions (`sections/actions.ts`) — upsert по `(page,section_key)` под cookie-сессией (RLS authenticated), `clampConfig` режет строки до 2000.
