...
- `src/components/calculator/Model3D.tsx`: чтобы убрать обрезку wireframe по краям карточки, `Bounds margin` поднят до `2.35`, а `OrbitControls minDistance` до `5`. Это намеренно дает больше воздуха вокруг машины, особенно в профильном ракурсе и после ручного zoom/rotate.
