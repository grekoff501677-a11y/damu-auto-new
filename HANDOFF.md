...
- `src/components/calculator/Model3D.tsx`: обрезка wireframe по краям карточки была не near-plane. `Bounds` убран; после построения рёбер модель центрируется в wrapper-группе и масштабируется по длиннейшей стороне до `MODEL_MAX_SIZE = 3.4`. Zoom снова разрешён близко (`OrbitControls minDistance = 2.8`), потому что пользователь хочет приближать модель.
