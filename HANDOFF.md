...
Логотип в правой части HERO (на мобиле — центр над заголовком). **Текущая версия (эксперимент)**: эмблема в естественной яркости + мягкое золотое свечение позади + **GradientTracing-кольцо** (`components/ui/gradient-tracing.tsx`, патчнут пропом `responsive` — скейлится через viewBox 600×600): золотой градиент-пульс пробегает по окружности (r=282), статичного трека нет (`baseColor="transparent"`).
- ⚠️ **Прежняя версия «комета + спотлайт-reveal»** (три conic-дуги с сужающимся хвостом, горячая голова, рассеянный спотлайт, выхватывающий лого из тени) живёт в git — коммит `aa758d7`. Если эксперимент не понравится: `git show aa758d7:src/components/shared/HeroLogo.tsx` → вернуть файл (+ вернуть строку про gradient-tracing в «мёртвый код»).
- ⚠️ Новый знак (бейдж DAMU): пользователь загрузил `.cdr` (нерендерим в вебе) и `.ai` **без PDF-совместимости** — Cloudinary отдаёт пустой 160-байтовый растр. Нужен пере-экспорт: Illustrator → Save As .ai с галочкой «Create PDF Compatible File», либо (лучше) SVG/PNG-экспорт → upload в Cloudinary как image. После этого замена — одна константа `LOGO` (с `f_auto,q_auto,w_700`).
